...
while (true) {
  ...
  const itemName = readlineSync.question('Enter item or press Enter to finish');
  ...
  const itemPrice = parseFloat(readlineSync.question('Enter item price: '));
  const itemQuantity = parseFloat(readlineSync.question('Enter item quantity: '));
